...
from bayaml import __author__, __version__, info
from bayaml.cli.commands import (
    run_automl_from_config,
    run_from_config,
    show_leaderboard,
    visualize_leaderboard,
)
from bayaml.integrations import bootstrap_integrations
...
def banner() -> str:
    return (
        ...
        f"Author  : {__author__}\n"
        "GitHub : https://github.com/adityassarode"
    )


def main() -> None:
    parser = argparse.ArgumentParser(
        prog="bayaml", description="Bayaml is a lightweight ML & AI orchestration framework that automates data loading, preprocessing, model training, evaluation, and deployment through a deterministic, privacy-safe Auto Mode with built-in AutoML. Built and maintained by Aditya Sarode."
    )
    ...
    if args.command == "info":
        ...
        if args.open_website:
            webbrowser.open("https://github.com/adityassarode/bayaml")
        return
